...
public class MaxBinarySearchSubtree {
    ...
    public static Info process(binaryTree node) {
        ...
        int maxBSTsize = 0;
        Treesize =
                (leftInfo == null ? 0 : leftInfo.Treesize) + (rightInfo == null ? 0 : rightInfo.Treesize) + 1;
    ...
    private static BSTinfo processBST(binaryTree head) {
        ...
        //处理本节点要向上反应的信息
        int max = Math.max((leftInfo == null ? head.data : leftInfo.max), (rightInfo == null ? head.data :
                rightInfo.max));

        int min = Math.min((leftInfo == null ? head.data : leftInfo.min), (rightInfo == null ? head.data :
                rightInfo.min));
